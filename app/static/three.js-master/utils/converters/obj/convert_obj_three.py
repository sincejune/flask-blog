        ...
        for i in indices:
            ...
            if normpath != norminfile or not skipOriginalMorph:
                ...
                if n_vertices != n_morph_vertices:
                    ...
                    print
                    "adding [%s] with %d vertices" % (name, n_morph_vertices)
    ...
    for mfilepattern in colorfiles.split():
        ...
        for path in matches:
            ...
            if n_vertices != n_morph_vertices:
                ...
                print
                "adding [%s] with %d face colors" % (name, len(morphFaceColors))
...
def convert_ascii(infile, morphfiles, colorfiles, outfile):
    ...
    print
    "%d vertices, %d faces, %d materials" % (len(vertices), len(faces), len(materials))
...
def usage():
    print
    "Usage: %s -i filename.obj -o filename.js [-m morphfiles*.obj] [-c morphcolors*.obj] [-a center|top|bottom] [-s flat|smooth] [-t binary|ascii] [-d invert|normal]" % os.path.basename(
        sys.argv[0])
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hbi:m:c:b:o:a:s:t:d:x:f:",
                                   ["help", "bakecolors", "input=", "morphs=", "colors=", "output=", "align=",
                                    "shading=", "type=", "truncatescale=", "framestep="])
    ...
    print
    "Converting [%s] into [%s] ..." % (infile, outfile)
