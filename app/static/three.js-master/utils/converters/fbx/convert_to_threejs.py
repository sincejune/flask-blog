...
class ChunkedIndent(object):
    ...
    def encode(self):
        ...
        if self.force_rounding:
            return ['{CHUNK}%s' % ', '.join(str(round(f, 6)) for f in self.value[i:i + self.size]) for i in
                    range(0, len(self.value), self.size)]
        else:
            return ['{CHUNK}%s' % ', '.join(str(f) for f in self.value[i:i + self.size]) for i in
                    range(0, len(self.value), self.size)]
...
def generate_material_object(material):
    ...
    if implementation:
        ...
        print
        "Unknown type of Material", getMaterialName(material)
...
def process_mesh_polygons(mesh_list, normals_to_indices, colors_to_indices, uvs_to_indices_list, vertex_offset_list,
                          material_offset_list):
    faces = []
    for mesh_index in range(len(mesh_list)):
        ...
        for poly_index in range(poly_count):
            ...
            face_normals = generate_normal_indices_for_poly(poly_index, normal_values, normal_indices,
                                                            normals_to_indices)
            ...
            for l in range(len(uv_indices_layers)):
                ...
                face_uv_indices = generate_uv_indices_for_poly(poly_index, uv_values, uv_indices,
                                                               uvs_to_indices_list[l])
...
def copy_textures(textures):
    ...
    for key in textures:
        ...
        try:
            ...
            print
            "I/O error({0}): {1} {2}".format(e.errno, e.strerror, url)
...
if __name__ == "__main__":
    ...
    parser.add_option('-n', '--no-texture-copy', action='store_true', dest='notexturecopy',
                      help="don't copy texture files", default=False)
    ...
    parser.add_option('-y', '--force-y-up', action='store_true', dest='forceyup',
                      help="ensure that the y axis shows up", default=False)
    ...
    if not result:
        ...
        if option_pretty_print:
            output_string = json.dumps(output_content, indent=4, cls=CustomEncoder, separators=(',', ': '),
                                       sort_keys=True)
