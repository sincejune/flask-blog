...
class ThreeJsWriter(object):
    ...
    def _getVertices(self):
        return [coord for mesh in self._allMeshes() for point in mesh.getPoints(space='world') for coord in
                [round(point.x, FLOAT_PRECISION), round(point.y, FLOAT_PRECISION), round(point.z, FLOAT_PRECISION)]]
    ...
    def _exportNormals(self, mesh):
        for normal in mesh.getNormals():
            self.normals += [round(normal.x, FLOAT_PRECISION), round(normal.y, FLOAT_PRECISION),
                             round(normal.z, FLOAT_PRECISION)]
    ...
    def _exportKeyframeAnimations(self):
        ...
        self.animations.append({
            ...
            "length": (playbackOptions(maxTime=True, query=True) - playbackOptions(minTime=True,
                                                                                   query=True)) / frameRate,
            "fps": 1,
            "hierarchy": hierarchy
        })
            ...
            if len(skins) > 0:
                ...
                for weights in skin.getWeights(mesh.vtx):
                    ...
                    if numWeights > self.options["influencesPerVertex"]:
                        raise Exception("More than " + str(
                            self.options["influencesPerVertex"]) + " influences on a vertex in " + mesh.name() + ".")
...
def initializePlugin(mobject):
    ...
    try:
        mplugin.registerFileTranslator(kPluginTranslatorTypeName, None, translatorCreator, kOptionScript,
